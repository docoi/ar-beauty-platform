...
fn vert_main_3d(input: VertexInput3D) -> VertexOutput3D {
  ...
  // For now, we simplify and assume uniform scaling, which allows using the model matrix directly.
  // This is a common simplification but can be improved for perfect lighting under non-uniform scale.
...
fn frag_main_3d(input: VertexOutput3D) -> @location(0) vec4f {
  // Sample the albedo (grayscale texture) and normal map.
  ...
  // Combine the grayscale albedo with the tint color selected by the user.
  ...
  // This ensures we get a coherent shape first.
  ...
  //    let N = normalize(tbnMatrix * tangentSpaceNormal);
