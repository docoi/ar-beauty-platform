// src/pages/LipstickMirrorLive.jsx (Applying successful TestWebGPUCanvas pattern - videoRef.current checked)
...
export default function LipstickMirrorLive() {
  ...
  const videoRef = useRef(null); // Correctly defined
  ...
  useEffect(() => {
    ...
    let currentLandmarker = null; 
    let resizeObserver = null; let renderLoopStarted = false;
    
    // Get DOM element refs early, but always check for .current inside functions
    ...
    const configureCanvas = (entries) => {
      // Use canvasRef.current inside here to ensure it's the latest if it could change (though it shouldn't for a top-level canvas)
    ...
    const render = async () => {
      const currentDevice = deviceRef.current; // Use refs for device/context in render
      const currentContext = contextRef.current;
      const currentVideoElement = videoRef.current; // Use videoRef.current

      if (!currentDevice || !currentContext || !pipelineStateRef.current.videoPipeline || !currentVideoElement) {
        animationFrameIdRef.current = requestAnimationFrame(render); return; 
      }
      frameCounter.current++;
      if (currentVideoElement.readyState < currentVideoElement.HAVE_ENOUGH_DATA || currentVideoElement.videoWidth === 0) {
        animationFrameIdRef.current = requestAnimationFrame(render); return;
      }
      
      if (pipelineStateRef.current.aspectRatioUniformBuffer && currentVideoElement.videoWidth > 0 && currentContext.canvas.width > 0) {
        const aspectRatioData = new Float32Array([ currentVideoElement.videoWidth, currentVideoElement.videoHeight, currentContext.canvas.width, currentContext.canvas.height ]);
        currentDevice.queue.writeBuffer(pipelineStateRef.current.aspectRatioUniformBuffer, 0, aspectRatioData);
      }

      let numLipVertices = 0;
      if (landmarker) { // Use landmarker from state
        try {
          const now = performance.now(); const results = landmarker.detectForVideo(currentVideoElement, now);
      ...
      try {
        videoTextureGPU = currentDevice.importExternalTexture({ source: currentVideoElement });
    ...
    const initializeAll = async () => {
      ...
      try {
        ...
        setLandmarker(lm); // Set landmarker state
        ...
        // Use canvasElement from top of useEffect scope
        ...
        // Use videoElement from top of useEffect scope
        ...
        setIsGpuReady(true); // Not strictly needed if render checks device/context directly
        console.log("[initializeAll] All systems Initialized.");
    ...
    return () => { /* ... cleanup ... */
      ...
      if (resizeHandlerRef.current && canvasRef.current) { /* If it was a window listener, remove it */ }
      ...
      setLandmarker(null); setIsGpuReady(false); // Reset state
    };
  }, []); // Main effect
