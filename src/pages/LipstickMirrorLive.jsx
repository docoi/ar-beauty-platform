// src/pages/LipstickMirrorLive.jsx (Restore to version with viewport/scissor, original import name)

import React, { useEffect, useRef, useState } from 'react';
import initWebGPU from '@/utils/initWebGPU'; // Original import name
...
export default function LipstickMirrorLive() {
  ...
  useEffect(() => {
    ...
    const initGPUAndVideo = async () => {
      try {
        ...
        const { device, context, format } = await initWebGPU(canvasRef.current); // Calls original initWebGPU
  ...
  const allResourcesReady = !!(landmarker && gpuResourcesReady && renderState.device && renderState.context); // Ensure context is also checked

  console.log('[COMPONENT_BODY_RENDER] allResourcesReady:', allResourcesReady, { lm: !!landmarker, gpuRdy: gpuResourcesReady, dev: !!renderState.device, ctx: !!renderState.context });
  ...
  useEffect(() => {
    ...
    const render = async () => {
      if (!renderState.device || !renderState.context) { console.warn(`[RENDER ${frameCounter.current}] Loop aborted: Device or Context lost.`); renderState.renderRequestId = null; return; }
  ...
  }, [allResourcesReady, landmarker]); // Removed webGPUDevice from deps as it's part of renderState or checked directly
