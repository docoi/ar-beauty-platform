// src/pages/LipstickMirrorLive_Clone.jsx (Using (pt.x - 0.5) * 2 for lips X-coord - "Test A" Base)
...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    console.log("[LML_Clone TestA_Final] useEffect running.");
    ...
    if (!canvasElement || !videoElement) {
      console.error("[LML_Clone TestA_Final] Canvas or Video element not available.");
    ...
    const configureCanvas = (entries) => {
      if (!deviceInternal || !contextInternal || !formatInternal || !canvasRef.current) { 
          console.warn("[LML_Clone TestA_Final configureCanvas] Prerequisites not met."); return; 
      }
      const currentCanvas = canvasRef.current;
      if (entries) { /* console.log("[LML_Clone TestA_Final configureCanvas via RO]"); */ } 
      else { /* console.log("[LML_Clone TestA_Final configureCanvas direct]"); */ }
      
      const dpr = window.devicePixelRatio || 1;
      const cw = currentCanvas.clientWidth; const ch = currentCanvas.clientHeight;
      if (cw === 0 || ch === 0) { console.warn(`[LML_Clone TestA_Final configureCanvas] Canvas clientW/H is zero.`); return; }
      const tw = Math.floor(cw * dpr); const th = Math.floor(ch * dpr);
      if (currentCanvas.width !== tw || currentCanvas.height !== th) { currentCanvas.width = tw; currentCanvas.height = th; console.log(`[LML_Clone TestA_Final configureCanvas] Canvas buffer SET:${tw}x${th}`); }
      try { 
        ...
        if (frameCounter.current < 2 || (entries && frameCounter.current % 60 === 1)) console.log(`[LML_Clone TestA_Final configureCanvas] Context CONFIG. Size:${currentCanvas.width}x${currentCanvas.height}`); 
      } catch (e) { console.error("[LML_Clone TestA_Final configureCanvas] Error config context:", e); setError("Error config context."); }
    };
    ...
    const render = async () => {
      ...
      if (activeLandmarker && pState.vertexBuffer) { 
        try { const now = performance.now(); const results = activeLandmarker.detectForVideo(currentVideoEl, now); 
          if (results?.faceLandmarks?.length > 0) {
            ...
            if (lips.length > 0) { 
                // "Test A" X-coordinate transformation (non-mirrored overlay for non-mirrored video)
                const lipVertexData = new Float32Array(lips.flat().map(pt => [(pt.x - 0.5) * 2, (0.5 - pt.y) * 2]).flat()); 
                numLipVertices = lipVertexData.length / 2; 
                if(lipVertexData.byteLength > 0){ if(lipVertexData.byteLength <= pState.vertexBufferSize) {currentDevice.queue.writeBuffer(pState.vertexBuffer, 0, lipVertexData);} else {numLipVertices = 0; console.warn("Lip vertex data too large for buffer.")}} else {numLipVertices = 0;}
            } else {numLipVertices = 0;}
          } else { numLipVertices = 0; }
        } catch (e) { numLipVertices = 0; console.error("Error in landmark processing:", e); }
      }
      ...
      if (frameCounter.current === 1 || frameCounter.current % 240 === 1) { console.log(`[LML_Clone TestA_Final RENDER ${frameCounter.current}] Canvas: ${canvasRef.current.width}x${canvasRef.current.height}. GPU Tex: ${currentGpuTexture.width}x${currentGpuTexture.height}`); }
      ...
      if(numLipVertices>0 && pState.lipstickPipeline && pState.vertexBuffer && pState.aspectRatioBindGroup){ passEnc.setPipeline(pState.lipstickPipeline); passEnc.setBindGroup(0, pState.aspectRatioBindGroup); passEnc.setVertexBuffer(0,pState.vertexBuffer); passEnc.draw(numLipVertices); if (frameCounter.current === 1 || frameCounter.current % 60 === 1) console.log(`[LML_Clone TestA_Final RENDER] Drawing ${numLipVertices} lip vertices.`); }
      passEnc.end(); currentDevice.queue.submit([cmdEnc.finish()]);
      if(frameCounter.current === 1) { console.log(`[LML_Clone TestA_Final RENDER 1] First full frame (video+lips).`); }
      animationFrameIdRef.current = requestAnimationFrame(render);
    }; 

    const initializeAll = async () => { 
        if (!navigator.gpu) { setError("WebGPU not supported."); return; }
        setDebugMessage("Initializing Final Test A...");
        try {
            console.log("[LML_Clone TestA_Final initializeAll] Initializing FaceLandmarker...");
            ...
            console.log("[LML_Clone TestA_Final initializeAll] FaceLandmarker ready.");

            console.log("[LML_Clone TestA_Final initializeAll] Initializing WebGPU Device & Format...");
            ...
            console.log("[LML_Clone TestA_Final initializeAll] Device obtained.");
            ...
            console.log("[LML_Clone TestA_Final initializeAll] Context and Format obtained.");

            console.log("[LML_Clone TestA_Final initializeAll] Creating pipelines and GPU resources...");
            ...
            console.log("[LML_Clone TestA_Final initializeAll] All Pipelines and GPU resources created.");
            
            console.log("[LML_Clone TestA_Final initializeAll] Setting up video element...");
            if (!navigator.mediaDevices?.getUserMedia) throw new Error("Camera not supported.");
            const stream = await navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480 } }); 
            videoElement.srcObject = stream; 
            await new Promise((res, rej) => { videoElement.onloadedmetadata = () => {res(); }; videoElement.onerror = () => rej(new Error("Video load error.")) }); 
            await videoElement.play(); 
            console.log("[LML_Clone TestA_Final initializeAll] Video playback started.");
            
            resizeObserverInternal = new ResizeObserver(resizeHandlerRef.current); 
            resizeObserverInternal.observe(canvasElement); 
            console.log("[LML_Clone TestA_Final initializeAll] ResizeObserver observing canvas.");
            if(resizeHandlerRef.current) resizeHandlerRef.current(); 
            
            console.log("[LML_Clone TestA_Final initializeAll] All sub-initializations complete.");
            if (!renderLoopStartedInternal) { render(); renderLoopStartedInternal = true; }
        } catch (err) { setError(`Init Test A Final failed: ${err.message.substring(0,50)}...`); console.error("[LML_Clone TestA_Final initializeAll] Major error:", err); }
    };
    initializeAll();
    return () => { /* ... Same cleanup ... */ 
        console.log("[LML_Clone TestA_Final MAIN_EFFECT_CLEANUP] Cleaning up.");
        ...
        deviceRef.current = null; contextRef.current = null; formatRef.current = null; landmarkerRef.current = null; setLandmarkerState(null); 
    };
  }, []);

  useEffect(() => { /* ... Same UI Message Effect ... */ 
    if(landmarkerState && deviceRef.current && contextRef.current && pipelineStateRef.current.lipstickPipeline && !error) { setDebugMessage("Test A Final Active"); } 
    else if (error) { setDebugMessage(`Error`); } else { setDebugMessage("Initializing Test A Final..."); }
