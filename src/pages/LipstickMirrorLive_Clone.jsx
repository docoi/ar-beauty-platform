// src/pages/LipstickMirrorLive_Clone.jsx (STAGE 1: Video Background - Built on working clear pattern)

import React, { useEffect, useRef, useState } from 'react';
import createPipelines from '@/utils/createPipelines'; // Ensure this is the version with aspectRatioGroupLayout
import lipTriangles from '@/utils/lipTriangles'; // Not used for drawing in this stage
...
export default function LipstickMirrorLive_Clone() {
  ...
  // Refs for WebGPU objects, these will be populated by the local vars in useEffect
  ...
  useEffect(() => {
    ...
    const configureCanvas = (entries) => {
      // These device, context, format are from the outer scope of initializeAll
      if (!device || !context || !format || !canvasRef.current) { 
          console.warn("[LML_Clone configureCanvas] Prerequisites not met."); return; 
      }
      const currentCanvas = canvasRef.current; // Use current ref inside handler
      if (entries) { console.log("[LML_Clone configureCanvas via RO]"); } 
      else { console.log("[LML_Clone configureCanvas direct]"); }
      ...
      if (currentCanvas.width !== tw || currentCanvas.height !== th) { 
          ...
          console.log(`[LML_Clone configureCanvas] Canvas buffer SET:${tw}x${th}`); 
      ...
      try { 
        ...
        console.log(`[LML_Clone configureCanvas] Context CONFIG. Size:${currentCanvas.width}x${currentCanvas.height}`); 
      }
    ...
    const render = async () => {
      // For render, best to use refs that were set once device/context are obtained
      const currentDevice = deviceRef.current; 
      const currentContext = contextRef.current;
      const currentVideoEl = videoRef.current; 
      const pState = pipelineStateRef.current;
      const activeLandmarker = landmarker; // From state
      ...
      } catch (e) { console.error(`[RENDER LML_Clone ${frameCounter.current}] Error import/bind texture:`, e); animationFrameIdRef.current = requestAnimationFrame(render); return; }
      ...
      if (pState.videoPipeline && frameBindGroupForTexture && pState.aspectRatioBindGroup) {
        ...
        passEnc.draw(6);
      } else { if (frameCounter.current % 60 === 1) { console.warn(`[RENDER LML_Clone ${frameCounter.current}] Skipping video draw: Missing resources.`); } }
    ...
    const initializeAll = async () => {
      ...
      try {
        ...
        device.lost.then((info) => { /* ... device lost handling ... */ 
            console.error(`[DEVICE_LOST_HANDLER LML_Clone] ${info.message}`);
            setError("Device Lost"); setDebugMessage("Error: Device Lost");
            if(animationFrameIdRef.current) cancelAnimationFrame(animationFrameIdRef.current);
            deviceRef.current = null; contextRef.current = null; setLandmarker(null);
        });
        ...
        // Debug message will be updated by the other useEffect
  ...
  useEffect(() => {
    if(landmarker && deviceRef.current && contextRef.current && pipelineStateRef.current.videoPipeline && !error) { 
        ...
        setDebugMessage(`Error: ${error.substring(0, 30)}...`); // Show only part of error
    } else {
