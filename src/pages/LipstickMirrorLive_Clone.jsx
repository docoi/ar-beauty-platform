...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    ...
    const render = async () => {
        ...
        if (!currentDevice || !currentContext || !pState.depthTextureView || (pState.lipModelData && !pState.lipModelPipeline) ) { animationFrameIdRef.current = requestAnimationFrame(render); return; }
        ...
        if (pState.lipModelMatrixUBO) {
            ...
            mat4.rotate(modelMatrix, modelMatrix, frameCounter.current * 0.01, [0, 1, 0]);
    ...
    const initializeAll = async () => {
        ...
        try {
            ...
            if (!gltfJson) throw new Error("GLTF JSON missing.");
            if (!gltfJson.meshes || gltfJson.meshes.length === 0) throw new Error("No meshes in GLTF JSON.");
            const meshJson = gltfJson.meshes[0];
            if (!meshJson.primitives || meshJson.primitives.length === 0) throw new Error("No primitives in mesh.");
        ...
        try {
            ...
            console.log("[LML_Clone 3DModel] GPU resources for 3D model initialized.");
