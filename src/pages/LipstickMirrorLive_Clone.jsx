// src/pages/LipstickMirrorLive_Clone.jsx (TEST A: Lip X-coord as (pt.x - 0.5) * 2)
...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    ...
    const configureCanvas = (entries) => { /* ... Same as your last fully working version ... */ 
      ...
      if (currentCanvas.width !== tw || currentCanvas.height !== th) { currentCanvas.width = tw; currentCanvas.height = th; console.log(`[LML_Clone TEST_A configureCanvas] Canvas buffer SET:${tw}x${th}`); }
      try { contextInternal.configure({ device: deviceInternal, format: formatInternal, alphaMode: 'opaque', size: [currentCanvas.width, currentCanvas.height] }); 
            /* console.log(`[LML_Clone TEST_A configureCanvas] Context CONFIG.`); */
    ...
    const render = async () => {
      ...
      let videoTextureGPU, frameBindGroupForTexture; try { /* ... */ } catch (e) { /* ... */ return; } // Assume this is fine
      ...
      // if (frameCounter.current === 1 || frameCounter.current % 240 === 1) { /* ... console log ... */ }
    ...
    const initializeAll = async () => { /* ... Same full initializeAll as your last working version ... */ 
        ...
        try {
            ...
            console.log("[LML_Clone TEST_A] All sub-initializations complete.");
            if (!renderLoopStartedInternal) { render(); renderLoopStartedInternal = true; }
        } catch (err) { setError(`Init Test A failed: ${err.message.substring(0,50)}...`); }
    };
    initializeAll();
    return () => { /* ... Same cleanup ... */ 
  ...
  useEffect(() => { /* ... Same UI Message Effect ... */ 
    if(landmarkerState && deviceRef.current && contextRef.current && pipelineStateRef.current.lipstickPipeline && !error) { setDebugMessage("Test A Active"); } 
    else if (error) { setDebugMessage(`Error`); } else { setDebugMessage("Initializing Test A..."); }
