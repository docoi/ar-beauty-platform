// src/pages/LipstickMirrorLive_Clone.jsx (Using (0.5 - pt.x) * 2 for lips X-coord to match UNMIRRORED video bg)
...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    console.log("[LML_Clone UnmirroredVid_UnmirroredLips] useEffect running."); // Updated log prefix
    ...
    const configureCanvas = (entries) => { /* ... Same as before ... */ 
      ...
      if (currentCanvas.width !== tw || currentCanvas.height !== th) { currentCanvas.width = tw; currentCanvas.height = th; console.log(`[LML_Clone UnmirroredVid_UnmirroredLips configureCanvas] Canvas buffer SET:${tw}x${th}`); }
      try { 
        ...
        if (frameCounter.current < 2 || (entries && frameCounter.current % 60 === 1)) console.log(`[LML_Clone UnmirroredVid_UnmirroredLips configureCanvas] Context CONFIG. Size:${currentCanvas.width}x${currentCanvas.height}`); 
      ...
      if (activeLandmarker && pState.vertexBuffer) { 
        try { const now = performance.now(); const results = activeLandmarker.detectForVideo(currentVideoEl, now); 
          if (results?.faceLandmarks?.length > 0) {
            ...
            if (lips.length > 0) { 
                // X-coordinate for UNMIRRORED overlay to match UNMIRRORED video background
    ...
    const initializeAll = async () => { /* ... Same full initializeAll as before ... */ 
        ...
        try {
            ...
            console.log("[LML_Clone UnmirroredVid_UnmirroredLips] All sub-initializations complete.");
            if (!renderLoopStartedInternal) { render(); renderLoopStartedInternal = true; }
        } catch (err) { setError(`Init UnmirroredLips failed: ${err.message.substring(0,50)}...`); console.error("[LML_Clone UnmirroredVid_UnmirroredLips initializeAll] Major error:", err); }
    };
    initializeAll();
    return () => { /* ... Same cleanup ... */ 
  ...
  useEffect(() => { /* ... Same UI Message Effect ... */ 
    if(landmarkerState && deviceRef.current && contextRef.current && pipelineStateRef.current.lipstickPipeline && !error) { setDebugMessage("Live Active (Unmirrored Video + Lips)"); } 
    else if (error) { setDebugMessage(`Error`); } else { setDebugMessage("Initializing (Unmirrored Video + Lips)..."); }
