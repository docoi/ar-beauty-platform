// src/pages/LipstickMirrorLive_Clone.jsx (Using (pt.x - 0.5) * 2 for lips X-coord)
...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    console.log("[LML_Clone TestA_CorrectedX] useEffect running.");
    ...
    if (!canvasElement || !videoElement) {
      console.error("[LML_Clone TestA_CorrectedX] Canvas or Video element not available.");
    ...
    const configureCanvas = (entries) => {
      if (!deviceInternal || !contextInternal || !formatInternal || !canvasRef.current) { 
          console.warn("[LML_Clone TestA_CorrectedX configureCanvas] Prerequisites not met."); return; 
      }
      ...
      if (cw === 0 || ch === 0) { console.warn(`[LML_Clone TestA_CorrectedX configureCanvas] Canvas clientW/H is zero.`); return; }
      const tw = Math.floor(cw * dpr); const th = Math.floor(ch * dpr);
      if (currentCanvas.width !== tw || currentCanvas.height !== th) { currentCanvas.width = tw; currentCanvas.height = th; console.log(`[LML_Clone TestA_CorrectedX configureCanvas] Canvas buffer SET:${tw}x${th}`); }
      try { 
        ...
        if (frameCounter.current < 2) console.log(`[LML_Clone TestA_CorrectedX configureCanvas] Context CONFIG. Size:${currentCanvas.width}x${currentCanvas.height}`); 
      } catch (e) { console.error("[LML_Clone TestA_CorrectedX configureCanvas] Error config context:", e); setError("Error config context."); }
    };
    ...
    const render = async () => {
      ...
      if (activeLandmarker && pState.vertexBuffer) { 
        try { const now = performance.now(); const results = activeLandmarker.detectForVideo(currentVideoEl, now); 
          if (results?.faceLandmarks?.length > 0) {
            ...
            if (lips.length > 0) { 
                // Using (pt.x - 0.5) * 2 for X-coordinate
      ...
      // if (frameCounter.current === 1 || frameCounter.current % 240 === 1) { /* console log canvas/texture size */ }
    ...
    const initializeAll = async () => { 
        ...
        try {
            ...
            console.log("[LML_Clone TestA_CorrectedX] FaceLandmarker ready.");
            ...
            console.log("[LML_Clone TestA_CorrectedX] Device, Context, Format obtained.");
            ...
            console.log("[LML_Clone TestA_CorrectedX] Pipelines & GPU resources created.");
            
            const stream = await navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480 } }); 
            videoElement.srcObject = stream; 
            await new Promise((res) => { videoElement.onloadedmetadata = () => {res(); }; }); 
            await videoElement.play(); 
            console.log("[LML_Clone TestA_CorrectedX] Video playback started.");
            ...
            console.log("[LML_Clone TestA_CorrectedX] All sub-initializations complete.");
            if (!renderLoopStartedInternal) { render(); renderLoopStartedInternal = true; }
        } catch (err) { setError(`Init Test A CorrectedX failed: ${err.message.substring(0,50)}...`); }
    };
    initializeAll();
    return () => { /* ... Same cleanup ... */ 
  ...
  useEffect(() => { /* ... Same UI Message Effect ... */ 
    if(landmarkerState && deviceRef.current && contextRef.current && pipelineStateRef.current.lipstickPipeline && !error) { setDebugMessage("Test A Active (CorrectedX)"); } 
    else if (error) { setDebugMessage(`Error`); } else { setDebugMessage("Initializing Test A (CorrectedX)..."); }
