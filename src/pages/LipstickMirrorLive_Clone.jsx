// src/pages/LipstickMirrorLive_Clone.jsx (TEST B: Lip X-coord as (0.5 - pt.x) * 2 - For MIRRORED VIDEO)
...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    console.log("[LML_Clone TEST_B_MirroredX] useEffect running."); // Log prefix updated
    ...
    const configureCanvas = (entries) => { /* ... Same as before ... */ 
      ...
      try { 
        ...
        if (frameCounter.current < 2) console.log(`[LML_Clone TEST_B configureCanvas] Context CONFIG. Size:${currentCanvas.width}x${currentCanvas.height}`); 
    ...
    const initializeAll = async () => { /* ... Same full initializeAll as Test A ... */ 
        ...
        setDebugMessage("Initializing Test B (MirroredX)..."); // Updated message
        try {
  ...
  useEffect(() => { /* ... Same UI Message Effect ... */ 
    if(landmarkerState && deviceRef.current && contextRef.current && pipelineStateRef.current.lipstickPipeline && !error) { setDebugMessage("Test B Active (MirroredX)"); } 
    else if (error) { setDebugMessage(`Error`); } else { setDebugMessage("Initializing Test B (MirroredX)..."); }
