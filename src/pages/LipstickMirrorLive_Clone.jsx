...
export default function LipstickMirrorLive_Clone() {
  ...
  const pipelineStateRef = useRef({
    ...
    aspectRatioGroupLayout: null, aspectRatioUniformBuffer: null, aspectRatioBindGroup: null, // Will hold Video Dims + MVP Matrix
    ...
    depthTexture: null, depthTextureView: null, // NEW for 3D depth
  });
  ...
  useEffect(() => {
    console.log("[LML_Clone 3DModel] Main useEffect (depth texture, MVP setup).");
    ...
    const configureCanvasAndDepthTexture = () => {
        ...
        try {
            contextInternal.configure({ device: deviceInternal, format: formatInternal, alphaMode: 'opaque', size: [targetWidth, targetHeight] });
        } catch (e) { setError("Error config context: " + e.message); console.error("Error config context:", e); return; }

        // (Re)create depth texture if size changed or not yet created
        ...
        if (needsReconfigure || !pState.depthTexture || pState.depthTexture.width !== targetWidth || pState.depthTexture.height !== targetHeight) {
            pState.depthTexture?.destroy(); // Destroy old one if exists
            pState.depthTexture = deviceInternal.createTexture({
                size: [targetWidth, targetHeight],
                format: 'depth24plus', // Must match format in pipeline depthStencil state
    ...
    resizeHandlerRef.current = configureCanvasAndDepthTexture; // Use the new combined function
    ...
    const render = async () => {
        ...
        // Ensure depth texture is up-to-date with canvas size before render pass
        if (pState.depthTexture.width !== currentContext.canvas.width || pState.depthTexture.height !== currentContext.canvas.height) {
            console.warn("[LML_Clone 3DModel Render] Depth texture size mismatch with canvas. Reconfiguring.");
            configureCanvasAndDepthTexture(); // This will recreate depth texture to match
            animationFrameIdRef.current = requestAnimationFrame(render); // Skip this frame, try next
            return;
        }
        ...
        if (pState.aspectRatioUniformBuffer) {
            ...
            currentDevice.queue.writeBuffer(pState.aspectRatioUniformBuffer, 0, videoDimData); // Video dims at offset 0

            // Basic MVP Matrix for 3D Model (simple orthographic for now, or identity if scaled in NDC)
            const projectionMatrix = mat4.create();
            const canvasAspectRatio = currentContext.canvas.width / currentContext.canvas.height;
            // Simple orthographic projection to map NDC-like coordinates (-1 to 1)
            mat4.ortho(projectionMatrix, -canvasAspectRatio, canvasAspectRatio, -1, 1, -1, 1); 
            // Or, for a simple pass-through if model is already in NDC: mat4.identity(projectionMatrix);
            
            const viewMatrix = mat4.create(); // Identity view matrix (camera at origin, looking down -Z)
            mat4.lookAt(viewMatrix, vec3.fromValues(0,0,0.5), vec3.fromValues(0,0,0), vec3.fromValues(0,1,0)); // Simple camera
            
            const modelMatrix = mat4.create(); // Identity model matrix (model at origin)
            // mat4.scale(modelMatrix, modelMatrix, [0.2, 0.2, 0.2]); // Example: Scale model down
            // mat4.translate(modelMatrix, modelMatrix, [0, -0.1, 0]); // Example: Move model down
            ...
            currentDevice.queue.writeBuffer(pState.aspectRatioUniformBuffer, 4 * 4, mvpMatrix as Float32Array); // MVP at offset 16 bytes (4 floats)
        }
        if (pState.lipstickMaterialUniformBuffer) { /* ... same ... */ }
        if (pState.lightingUniformBuffer) { /* ... same ... */ }
        
        let videoTextureGPU, frameBindGroupForTexture; /* ... same ... */
        ...
        let currentGpuTexture, currentTextureView; /* Renamed texView to currentTextureView */
        ...
        const passEnc = cmdEnc.beginRenderPass({ 
            colorAttachments: [{ 
                view: currentTextureView, // Use the view from getCurrentTexture()
            ...
            depthStencilAttachment: { // NEW
                ...
                depthClearValue: 1.0,
                depthLoadOp: 'clear',
                depthStoreOp: 'store', // or 'discard' if not needed after
            }
        });
        ...
        if (pState.videoPipeline && frameBindGroupForTexture && pState.aspectRatioBindGroup) { /* ... same ... */ }
        if (pState.lipModelPipeline && pState.lipModelVertexBuffer && pState.lipModelIndexBuffer && pState.lipModelNumIndices > 0 && pState.lipModelAspectRatioBindGroup && pState.lipModelMaterialBindGroup && pState.lipModelLightingBindGroup) { /* ... same ... */ }
    ...
    const initializeAll = async () => {
        ...
        } catch (modelLoadError) { console.error("[LML_Clone 3DModel] Error loading/processing lip model:", modelLoadError); setError(`Failed to load 3D lip model: ${modelLoadError.message.substring(0, 150)}...`); setDebugMessage("Error: 3D Model Load Failed."); return;  }
        ...
        try {
            ...
            // Initial canvas and depth texture setup
            ...
            if (!layoutsAndPipelines.videoPipeline || !layoutsAndPipelines.lipModelPipeline) {
                // Log specific pipeline failure
                if (!layoutsAndPipelines.videoPipeline) console.error("Video pipeline creation failed in returned object.");
                if (!layoutsAndPipelines.lipModelPipeline) console.error("Lip model pipeline creation failed in returned object.");
                throw new Error("Pipeline creation failed (video or lip model missing).");
            }
            ...
            // Uniform buffer for Video Dims (4 floats) + MVP Matrix (16 floats) = 20 floats
            p.aspectRatioUniformBuffer = deviceInternal.createBuffer({ label: "VideoDims_MVP_UB", size: 20 * 4, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
            ...
            if (p.lipstickMaterialGroupLayout && p.lipstickMaterialUniformBuffer ) { p.lipModelMaterialBindGroup = deviceInternal.createBindGroup({label:"3D Lip Material BG", layout:p.lipstickMaterialGroupLayout, entries: materialEntries}); } else { throw new Error("Material bind group creation failed for 3D model."); }
            ...
            if (model && model.positions && model.normals && model.uvs && model.indices) {
                ...
                p.lipModelVertexBuffer = deviceInternal.createBuffer({ label: "3D Lip Model VB", size: interleavedBufferData.byteLength, usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST });
                deviceInternal.queue.writeBuffer(p.lipModelVertexBuffer, 0, interleavedBufferData);

                p.lipModelIndexBuffer = deviceInternal.createBuffer({ label: "3D Lip Model IB", size: model.indices.byteLength, usage: GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST });
                ...
                console.log(`[LML_Clone 3DModel] Created VB (${numVertices} verts) and IB (${p.lipModelNumIndices} indices, format ${p.lipModelIndexFormat}) for 3D model using writeBuffer.`);
            } else { 
                let missing = [];
            ...
            resizeObserverInternal = new ResizeObserver(resizeHandlerRef.current); // resizeHandlerRef now points to configureCanvasAndDepthTexture
            resizeObserverInternal.observe(canvasElement);
            // Initial call to configure canvas and depth texture after device is ready
            configureCanvasAndDepthTexture(); 

            console.log("[LML_Clone 3DModel] GPU resources and video initialized (using writeBuffer for model).");
            setDebugMessage("Ready (3D Model Mode).");
            if (!renderLoopStartedInternal) { render(); renderLoopStartedInternal = true; }
        } catch (err) { setError(`GPU Init failed: ${err.message.substring(0,150)}...`); console.error("[LML_Clone 3DModel] Major error during GPU initialization:", err); setDebugMessage("Error: GPU Init Failed."); }
    ...
    return () => { 
        ...
        pState.depthTexture?.destroy(); // NEW: destroy depth texture
