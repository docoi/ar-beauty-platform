// src/pages/LipstickMirrorLive_Clone.jsx (TEST A: Lip X-coord as (pt.x - 0.5) * 2 - FULL CORRECTED CODE)
...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    console.log("[LML_Clone TEST_A_X_Standard] useEffect running.");
    ...
    if (!canvasElement || !videoElement) {
      console.error("[LML_Clone TEST_A_X_Standard] Canvas or Video element not available.");
    ...
    const configureCanvas = (entries) => {
      if (!deviceInternal || !contextInternal || !formatInternal || !canvasRef.current) { 
          console.warn("[LML_Clone TEST_A configureCanvas] Prerequisites not met."); return; 
      }
      const currentCanvas = canvasRef.current;
      if (entries) { /* console.log("[LML_Clone TEST_A configureCanvas via RO]"); */ } 
      else { /* console.log("[LML_Clone TEST_A configureCanvas direct]"); */ }
      
      const dpr = window.devicePixelRatio || 1;
      const cw = currentCanvas.clientWidth; const ch = currentCanvas.clientHeight;
      if (cw === 0 || ch === 0) { console.warn(`[LML_Clone TEST_A configureCanvas] Canvas clientW/H is zero.`); return; }
      const tw = Math.floor(cw * dpr); const th = Math.floor(ch * dpr);
      // console.log(`[LML_Clone TEST_A configureCanvas] DPR:${dpr}, clientW/H:${cw}x${ch} => phys:${tw}x${th}`);
      if (currentCanvas.width !== tw || currentCanvas.height !== th) { 
          currentCanvas.width = tw; currentCanvas.height = th; 
          console.log(`[LML_Clone TEST_A configureCanvas] Canvas buffer SET:${tw}x${th}`); 
      }
      // else { console.log(`[LML_Clone TEST_A configureCanvas] Canvas size ${tw}x${th} OK.`); }
      try { 
        ...
        // console.log(`[LML_Clone TEST_A configureCanvas] Context CONFIG. Size:${currentCanvas.width}x${currentCanvas.height}`); 
      }
      catch (e) { console.error("[LML_Clone TEST_A configureCanvas] Error config context:", e); setError("Error config context."); }
    };
    ...
    const render = async () => {
      ...
      if (!currentDevice || !currentContext || 
          !pState.videoPipeline || !pState.lipstickPipeline || 
          !pState.aspectRatioBindGroup || 
          !currentVideoEl) {
      ...
      if (activeLandmarker && pState.vertexBuffer) { 
        try { 
          ...
          if (results?.faceLandmarks?.length > 0) {
            ...
            if (lips.length > 0) { 
                ...
                if(lipVertexData.byteLength > 0){ 
                    if(lipVertexData.byteLength <= pState.vertexBufferSize) {
                        ...
                        console.warn(`[LML_Clone TEST_A RENDER] Lip vertex data too large.`);
                        numLipVertices = 0;
                    }
                } else {
                    numLipVertices = 0;
            ...
            console.error(`[LML_Clone TEST_A RENDER] Error in landmarker processing:`, e);
            numLipVertices = 0; 
        }
      }
      ...
      if (frameCounter.current === 1 || frameCounter.current % 240 === 1) { 
          console.log(`[LML_Clone TEST_A RENDER ${frameCounter.current}] Canvas: ${canvasRef.current.width}x${canvasRef.current.height}. GPU Tex: ${currentGpuTexture.width}x${currentGpuTexture.height}`); 
      }

      const cmdEnc = currentDevice.createCommandEncoder({label: "LML_Clone_TestA_Encoder"});
      ...
      if(numLipVertices>0 && pState.lipstickPipeline && pState.vertexBuffer && pState.aspectRatioBindGroup){ 
          ...
          if (frameCounter.current % 60 === 1) console.log(`[LML_Clone TEST_A RENDER] Drawing ${numLipVertices} lip vertices.`); 
      }
      passEnc.end(); 
      currentDevice.queue.submit([cmdEnc.finish()]);
      if(frameCounter.current === 1) { console.log(`[LML_Clone TEST_A RENDER 1] First full frame (video+lips).`); }
      animationFrameIdRef.current = requestAnimationFrame(render);
    }; 

    const initializeAll = async () => { 
        if (!navigator.gpu) { setError("WebGPU not supported."); return; }
        setDebugMessage("Initializing Test A...");
        try {
            console.log("[LML_Clone TEST_A initializeAll] Initializing FaceLandmarker...");
            ...
            console.log("[LML_Clone TEST_A initializeAll] FaceLandmarker ready.");

            console.log("[LML_Clone TEST_A initializeAll] Initializing WebGPU Device & Format...");
            ...
            console.log("[LML_Clone TEST_A initializeAll] Device obtained.");
            ...
            console.log("[LML_Clone TEST_A initializeAll] Context and Format obtained.");

            console.log("[LML_Clone TEST_A initializeAll] Creating pipelines and GPU resources...");
            ...
            console.log("[LML_Clone TEST_A initializeAll] All Pipelines and GPU resources created.");
            
            console.log("[LML_Clone TEST_A initializeAll] Setting up video element...");
            if (!navigator.mediaDevices?.getUserMedia) throw new Error("Camera not supported.");
            const stream = await navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480 } });
            videoElement.srcObject = stream;
            await new Promise((res, rej) => { 
                videoElement.onloadedmetadata = () => { console.log(`[LML_Clone TEST_A initializeAll] Video metadata: ${videoElement.videoWidth}x${videoElement.videoHeight}`); res(); };
                videoElement.onerror = () => rej(new Error("Video load error."));
            });
            await videoElement.play(); console.log("[LML_Clone TEST_A initializeAll] Video playback started.");
            
            resizeObserverInternal = new ResizeObserver(resizeHandlerRef.current);
            resizeObserverInternal.observe(canvasElement);
            console.log("[LML_Clone TEST_A initializeAll] ResizeObserver observing canvas.");
            if(resizeHandlerRef.current) resizeHandlerRef.current(); 
            else console.error("[LML_Clone TEST_A initializeAll] resizeHandlerRef.current is null");
            
            console.log("[LML_Clone TEST_A initializeAll] All sub-initializations complete.");
            if (!renderLoopStartedInternal) { console.log("[LML_Clone TEST_A initializeAll] Starting render loop."); render(); renderLoopStartedInternal = true; }
        } catch (err) { console.error("[LML_Clone TEST_A initializeAll] Major error:", err); setError(`Init Test A failed: ${String(err.message).substring(0,50)}...`); }
    };
    initializeAll();
    return () => { 
        console.log("[LML_Clone TEST_A MAIN_EFFECT_CLEANUP] Cleaning up.");
