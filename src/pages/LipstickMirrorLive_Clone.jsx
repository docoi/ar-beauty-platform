// src/pages/LipstickMirrorLive_Clone.jsx (TEST B: Lip X-coord as (0.5 - pt.x) * 2 - FULL CODE)
...
export default function LipstickMirrorLive_Clone() {
  ...
  useEffect(() => {
    console.log("[LML_Clone TEST_B_X_Mirrored] useEffect running."); // Updated log prefix
    ...
    if (!canvasElement || !videoElement) {
      console.error("[LML_Clone TEST_B] Canvas or Video element not available.");
    ...
    const configureCanvas = (entries) => {
      if (!deviceInternal || !contextInternal || !formatInternal || !canvasRef.current) { 
          console.warn("[LML_Clone TEST_B configureCanvas] Prerequisites not met."); return; 
      }
      const currentCanvas = canvasRef.current;
      if (entries) { /* console.log("[LML_Clone TEST_B configureCanvas via RO]"); */ } 
      else { /* console.log("[LML_Clone TEST_B configureCanvas direct]"); */ }
      
      const dpr = window.devicePixelRatio || 1;
      const cw = currentCanvas.clientWidth; const ch = currentCanvas.clientHeight;
      if (cw === 0 || ch === 0) { console.warn(`[LML_Clone TEST_B configureCanvas] Canvas clientW/H is zero.`); return; }
      const tw = Math.floor(cw * dpr); const th = Math.floor(ch * dpr);
      if (currentCanvas.width !== tw || currentCanvas.height !== th) { currentCanvas.width = tw; currentCanvas.height = th; console.log(`[LML_Clone TEST_B configureCanvas] Canvas buffer SET:${tw}x${th}`); }
      try { 
      ...
      if (activeLandmarker && pState.vertexBuffer) { 
        try { const now = performance.now(); const results = activeLandmarker.detectForVideo(currentVideoEl, now); 
          if (results?.faceLandmarks?.length > 0) {
            ...
            if (lips.length > 0) { 
                // TEST B: Mirrored X mapping for lips
    ...
    const initializeAll = async () => { /* ... Same full initializeAll as Test A ... */ 
        ...
        try {
            ...
            console.log("[LML_Clone TEST_B] All sub-initializations complete.");
            if (!renderLoopStartedInternal) { render(); renderLoopStartedInternal = true; }
        } catch (err) { setError(`Init Test B failed: ${err.message.substring(0,50)}...`); }
    };
    initializeAll();
    return () => { /* ... Same cleanup ... */ 
  ...
  useEffect(() => { /* ... Same UI Message Effect ... */ 
    if(landmarkerState && deviceRef.current && contextRef.current && pipelineStateRef.current.lipstickPipeline && !error) { setDebugMessage("Test B Active"); } 
    else if (error) { setDebugMessage(`Error`); } else { setDebugMessage("Initializing Test B..."); }
