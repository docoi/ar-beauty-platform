// src/components/StaticSelfieTryOn.jsx - Accepts Intensity Prop

import React, { useState, useRef, useEffect, useCallback } from 'react';
import TryOnRenderer from './TryOnRenderer';

// --- Accept effectIntensity prop ---
const StaticSelfieTryOn = ({
    faceLandmarker,
    effectIntensity // Accept intensity prop
    // Removed brightness/contrast props
}) => {
  ...
  useEffect(() => {
    if (!isPreviewing && rendererRef.current && staticImageRef.current) {
        if (!isDetecting) {
            console.log("Render Trigger: Calling renderStaticImageResults w/ intensity.");
            // Pass effect intensity to the renderer
            // Brightness/Contrast are handled internally by renderer now for static images
            rendererRef.current.renderStaticImageResults(
                staticImageRef.current,
                detectedSelfieResults,
                effectIntensity // Pass intensity prop
            );
        } else {
             console.log("Render Trigger: Rendering base image while detecting.");
             // Render base image while detecting (no results, pass intensity)
             rendererRef.current.renderStaticImageResults(
                 staticImageRef.current,
                 null,
                 effectIntensity
    ...
    // Add effectIntensity to dependencies
