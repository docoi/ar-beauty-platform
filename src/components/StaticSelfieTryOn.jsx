// src/components/StaticSelfieTryOn.jsx - FINAL (Hardcoded B/C Values)
...
const StaticSelfieTryOn = forwardRef(({
  ...
  useEffect(() => {
    if (!isPreviewing && rendererRef.current && staticImageRef.current) {
        // *** Use FIXED values ***
        const brightness = FIXED_SELFIE_BRIGHTNESS;
        const contrast = FIXED_SELFIE_CONTRAST;

        if (!isDetecting) {
            console.log("Render Trigger: Calling renderStaticImageResults w/ FIXED values.");
            // Call handle even if shader isn't using B/C yet, it sets internal state
            rendererRef.current.renderStaticImageResults(
                staticImageRef.current,
                detectedSelfieResults,
                brightness,          // Pass FIXED brightness
                contrast,            // Pass FIXED contrast
                effectIntensity
            );
        } else {
             console.log("Render Trigger: Rendering base image while detecting w/ FIXED values.");
             rendererRef.current.renderStaticImageResults(
                 staticImageRef.current,
                 null,
                 brightness,          // Pass FIXED brightness
         ...
         console.log("Render Trigger: Clearing canvas for preview (StaticSelfieTryOn).");
         rendererRef.current.clearCanvas();
    }
  }, [isPreviewing, isDetecting, detectedSelfieResults, selfieDimensions, effectIntensity, faceLandmarker]);

  // Retake Selfie
  const handleRetakeSelfie = () => {
    setIsPreviewing(true); setCapturedSelfieDataUrl(null); setDetectedSelfieResults(null); staticImageRef.current = null; setCameraError(null); setIsCameraLoading(true); setIsDetecting(false); setDebugInfo('');
   };
  ...
  return (
    <div className="border p-4 rounded bg-green-50">
      ...
      {isPreviewing ? ( /* Preview Mode */ <> {isCameraLoading && <p className="text-center py-4">Starting camera...</p>} {cameraError && <p className="text-red-500 text-center py-4">{cameraError}</p>} <div className="relative w-full max-w-md mx-auto aspect-[9/16] bg-gray-200 overflow-hidden rounded shadow"> <video ref={selfieVideoRef} autoPlay playsInline muted className={`absolute top-0 left-0 w-full h-full ${isCameraLoading || cameraError ? 'opacity-0' : 'opacity-100'}`} style={{ transform: 'scaleX(-1)', transition: 'opacity 0.3s', objectFit: 'cover' }}></video> {(isCameraLoading || cameraError) && ( <div className="absolute inset-0 flex items-center justify-center"><p className="text-gray-500 bg-white px-2 py-1 rounded shadow">{cameraError ? 'Error' : 'Loading...'}</p></div> )} </div> <div className="text-center mt-4"> <button onClick={handleTakeSelfie} disabled={isCameraLoading || !!cameraError || !cameraStream} className="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600 disabled:bg-gray-400 disabled:cursor-not-allowed"> Take Selfie </button> </div> </>
      ) : ( /* Captured Mode */ <> <div className="relative w-full max-w-md mx-auto bg-gray-200 overflow-hidden rounded shadow" style={{ paddingTop: `${selfieDimensions.height && selfieDimensions.width ? (selfieDimensions.height / selfieDimensions.width) * 100 : 75}%` }}> {selfieDimensions.width > 0 ? ( <TryOnRenderer ref={rendererRef} videoWidth={selfieDimensions.width} videoHeight={selfieDimensions.height} className="absolute top-0 left-0 w-full h-full" /> ) : ( <div className="absolute inset-0 flex items-center justify-center"><p className="text-gray-500">Loading Image...</p></div> )} </div> <div className="mt-2 p-2 border bg-gray-100 text-xs overflow-auto max-h-20 max-w-md mx-auto rounded"> <p className="font-semibold mb-1">Debug Info:</p> <pre className="whitespace-pre-wrap break-words">{isDetecting ? 'Analyzing selfie...' : (debugInfo || 'N/A')}</pre> </div> <div className="text-center mt-4"> <button onClick={handleRetakeSelfie} className="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600"> Retake Selfie </button> </div> </>
      )}
