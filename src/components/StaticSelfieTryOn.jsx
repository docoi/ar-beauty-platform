...
// const FIXED_SELFIE_BRIGHTNESS = 1.0; // Removed
// const FIXED_SELFIE_CONTRAST = 1.0; // Removed
...
const StaticSelfieTryOn = forwardRef(({
  ...
  const [isDetecting, setIsDetecting] = useState(false); // Use this state again
  ...
  useEffect(() => {
   ...
   }, [isPreviewing, faceLandmarker]); // Depends on landmarker for readiness
  ...
  const handleTakeSelfie = useCallback(() => {
    ...
    setCapturedSelfieDataUrl(dataUrl); setIsPreviewing(false);
    setDetectedLandmarkResults(null); setDetectedSegmentationResults(null); // Clear previous results
  ...
  useEffect(() => {
    if (!capturedSelfieDataUrl || !faceLandmarker || !imageSegmenter) {
        if (staticImageElement) setStaticImageElement(null);
        if (isDetecting) setIsDetecting(false);
        return;
    }
    console.log("StaticSelfieTryOn: Loading image for detection/segmentation...");
    ...
    imageElement.onload = () => {
      ...
      setStaticImageElement(imageElement); setDebugInfo('Image loaded, running AI...');
      try {
        if (faceLandmarker && imageSegmenter) {
          ...
          console.log(`StaticSelfieTryOn: AI Processing took ${endTime - startTime}ms`);
          setDetectedLandmarkResults(landmarkResults);
          setDetectedSegmentationResults(segmentationResults);
          setDebugInfo(`Analysis complete: ${landmarkResults?.faceLandmarks?.[0]?.length || 0} landmarks found.`);
        } else {
            setDetectedLandmarkResults(null); setDetectedSegmentationResults(null);
            setDebugInfo('AI models not ready for analysis.');
         }
      } catch(err) {
          console.error("StaticSelfieTryOn: Error during AI processing:", err);
          setDetectedLandmarkResults(null); setDetectedSegmentationResults(null);
          setDebugInfo(`AI Error: ${err.message}`);
       }
  ...
  }, [capturedSelfieDataUrl, faceLandmarker, imageSegmenter]); // Depends on all
          ...
          <div className="relative w-full max-w-md mx-auto bg-gray-200 overflow-hidden rounded shadow" style={{ paddingTop: `${selfieDimensions.height && selfieDimensions.width ? (selfieDimensions.height / selfieDimensions.width) * 100 : 75}%` }}>
            {staticImageElement ? (
              <TryOnRenderer
                ...
                mediaPipeResults={detectedLandmarkResults} // <<< Pass landmark results
