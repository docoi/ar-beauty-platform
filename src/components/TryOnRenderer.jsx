// src/components/TryOnRenderer.jsx - BASELINE WEBGL RENDER ONLY (Transparent Background - CORRECTED)
...
const TryOnRenderer = forwardRef(({
    ...
    useEffect(() => { /* Video Texture Effect */
        const videoElement = videoRefProp?.current; const material = planeMeshRef.current?.material; if (!isStatic && videoElement && videoElement.readyState >= videoElement.HAVE_METADATA) { if (!videoTextureRef.current || videoTextureRef.current.image !== videoElement) { videoTextureRef.current?.dispose(); videoTextureRef.current = new THREE.VideoTexture(videoElement); videoTextureRef.current.colorSpace = THREE.SRGBColorSpace; if (material) { material.map = videoTextureRef.current; material.needsUpdate = true; } } } else if (!isStatic && videoTextureRef.current) { if (material?.map === videoTextureRef.current) { material.map = null; material.needsUpdate = true; } videoTextureRef.current.dispose(); videoTextureRef.current = null; }
    }, [isStatic, videoRefProp, videoRefProp?.current?.readyState]);
    useEffect(() => { /* Image Texture Effect */
        const material = planeMeshRef.current?.material; if (isStatic && imageElement && imageElement.complete && imageElement.naturalWidth > 0) { if (!imageTextureRef.current || imageTextureRef.current.image !== imageElement) { imageTextureRef.current?.dispose(); imageTextureRef.current = new THREE.Texture(imageElement); imageTextureRef.current.colorSpace = THREE.SRGBColorSpace; imageTextureRef.current.needsUpdate = true; if (material) { material.map = imageTextureRef.current; material.needsUpdate = true; } } } else if (isStatic && imageTextureRef.current) { if (material?.map === imageTextureRef.current) { material.map = null; material.needsUpdate = true; } imageTextureRef.current.dispose(); imageTextureRef.current = null; }
    ...
    // --- Render Loop --- (No change)
     const renderLoop = useCallback(() => {
        animationFrameHandle.current = requestAnimationFrame(renderLoop); if (!isInitialized.current || !rendererInstanceRef.current || !sceneRef.current || !cameraRef.current || !planeMeshRef.current?.material) return;
        try { const mesh = planeMeshRef.current; const material = mesh.material; let sourceWidth = 0, sourceHeight = 0; let currentTexture = null; const isVideo = !isStatic; if (isVideo && videoTextureRef.current) { currentTexture = videoTextureRef.current; const video = currentTexture.image; if (video?.readyState >= 2) { sourceWidth = video.videoWidth; sourceHeight = video.videoHeight; } } else if (isStatic && imageTextureRef.current) { currentTexture = imageTextureRef.current; const image = currentTexture.image; if (image?.complete) { sourceWidth = image.naturalWidth; sourceHeight = image.naturalHeight; } } if (material.map !== currentTexture) { material.map = currentTexture; material.needsUpdate = true; } const planeVisible = !!material.map && sourceWidth > 0 && sourceHeight > 0; if (planeVisible) { fitPlaneToCamera(sourceWidth, sourceHeight); const scaleX = Math.abs(mesh.scale.x); const newScaleX = isVideo ? -scaleX : scaleX; if(mesh.scale.x !== newScaleX) { mesh.scale.x = newScaleX; } } else { if (mesh.scale.x !== 0 || mesh.scale.y !== 0) { mesh.scale.set(0, 0, 0); } } if (currentTexture?.needsUpdate) { currentTexture.needsUpdate = false; } rendererInstanceRef.current.render(sceneRef.current, cameraRef.current);
    ...
    // --- Initialization (ADDED TRANSPARENCY OPTIONS) ---
    const initThreeScene = useCallback(() => {
        console.log("TryOnRenderer (WebGL Base + Transparency): initThreeScene START"); if (!canvasRef.current || isInitialized.current) return;
        try {
            ...
            // <<< REMOVED setClearColor >>> Default clear color alpha is 0 when alpha is true
            ...
            const planeGeometry = new THREE.PlaneGeometry(1, 1);
             ...
             const planeMaterial = new THREE.MeshBasicMaterial({
                 map: null, side: THREE.DoubleSide, color: 0xffffff,
                 transparent: true
             });

            planeMeshRef.current = new THREE.Mesh(planeGeometry, planeMaterial); sceneRef.current.add(planeMeshRef.current);
            isInitialized.current = true; handleResize(); cancelAnimationFrame(animationFrameHandle.current); animationFrameHandle.current = requestAnimationFrame(renderLoop); console.log("TryOnRenderer (WebGL Base + Transparency): initThreeScene SUCCESSFUL.");
        } catch (error) { console.error("TryOnRenderer (WebGL Base + Transparency): initThreeScene FAILED:", error); isInitialized.current = false; }
