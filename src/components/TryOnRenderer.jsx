// src/components/TryOnRenderer.jsx - FINAL BBOX CONSTRAINT
...
const TryOnRenderer = forwardRef(({
    ...
    const customFragmentShader = `
        ...
        void main() {
            ...
            if(uHasLandmarks && uHasMask && uEffectIntensity > 0.01) {
                // Check if pixel is inside the calculated face bounding box
                if (isInsideFaceBBox(vUv, uFlipMaskX)) {
                    // If inside bbox, check the silhouette mask value
            ...
            if (applyEffect) {
                // Apply effect fully if within bbox and silhouette mask
                fC = applyHydrationEffect(fC);
                // Optional: Use mask value for smoother edges if desired later
                // float blendAmount = smoothstep(0.3, 0.8, silhouetteMaskValue) * uEffectIntensity;
                // fC = mix(bC.rgb, hydratedColor, blendAmount);
            }
    ...
    const HydrationShader = useRef({
        uniforms: {
            'tDiffuse': { value: null }, 'uSegmentationMask': { value: null },
            'uLandmarkData': { value: null }, 'uEffectIntensity': { value: 0.5 },
            'uHasMask': { value: false }, 'uHasLandmarks': { value: false },
    ...
    // --- Render Loop (Updates Both Mask and Landmark Uniforms) ---
