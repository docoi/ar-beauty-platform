// src/components/TryOnRenderer.jsx - Reads mask from FaceLandmarker results
...
const TryOnRenderer = forwardRef(({
    ...
    mediaPipeResults, // <<< NOW CONTAINS landmarks AND segmentationMasks
    // segmentationResults, // <<< REMOVED prop
    isStatic,
    effectIntensity,
    className, style
    // Removed brightness/contrast props as they weren't used
 }, ref) => {
    ...
    const imageTextureRef = useRef(null); const segmentationTextureRef = useRef(null); // Still used for the mask DataTexture
    ...
    const customFragmentShader = `
        ...
        void main() { vec4 bC=texture2D(tDiffuse,vUv); vec3 fC=bC.rgb; if(uHasMask&&uEffectIntensity>0.01){ float mV=texture2D(uSegmentationMask,vec2(vUv.x,1.0-vUv.y)).r; vec3 hC=applyHydrationEffect(fC); float bA=smoothstep(0.3,0.8,mV)*uEffectIntensity; fC=mix(fC,hC,bA); } fC=clamp(fC,0.0,1.0); gl_FragColor=vec4(fC,bC.a); }`;
    ...
    // --- ***** Segmentation Mask Texture Effect (READ FROM mediaPipeResults.segmentationMasks) ***** ---
    useEffect(() => {
        // *** Read from mediaPipeResults ***
        const results = mediaPipeResults;
        // *** Access segmentationMasks array ***
        const hasMaskData = results?.segmentationMasks?.[0];

        if (hasMaskData) {
            // *** Get the mask object from segmentationMasks ***
            const maskObject = results.segmentationMasks[0];
            const maskWidth = maskObject?.width;
            const maskHeight = maskObject?.height;
            let maskData = null;

            // Robustly get the mask data (assuming structure is similar)
            try {
                if (typeof maskObject?.getAsFloat32Array === 'function') {
                    maskData = maskObject.getAsFloat32Array();
                 } else {
                    // Fallback or error if the expected method isn't there
                    console.warn("TryOnRenderer: segmentationMasks[0] does not have getAsFloat32Array(). Check results structure.");
                    // Attempt direct access if it's just the array (less likely)
                    if (maskObject instanceof Float32Array) maskData = maskObject;
                 }
            } catch (error) {
                 console.error("TryOnRenderer: Error getting mask data from segmentationMasks:", error);
                 maskData = null;
            }
            ...
            if (maskData instanceof Float32Array && maskWidth > 0 && maskHeight > 0) {
                ...
                const throttleThreshold = isStatic ? 0 : 66;
                ...
                if (timeSinceLastUpdate > throttleThreshold) {
                    ...
                    try {
                        ...
                        if (!texture || texture.image.width !== maskWidth || texture.image.height !== maskHeight) {
                            texture?.dispose();
                            console.log(`TryOnRenderer Mask Texture: Creating NEW DataTexture from FaceLandmarker (${maskWidth}x${maskHeight})`); // Log source
                            ...
                            // console.log(`TryOnRenderer Mask Texture: Updating existing DataTexture from FaceLandmarker.`); // Log source
                        ...
                        console.error("TryOnRenderer: Error processing mask texture from FaceLandmarker:", error);
                ...
                // console.warn("TryOnRenderer: Invalid mask data from FaceLandmarker.");
                if (segmentationTextureRef.current) { segmentationTextureRef.current.dispose(); segmentationTextureRef.current = null; }
                 if (effectPassRef.current) { effectPassRef.current.uniforms.uSegmentationMask.value = null; effectPassRef.current.uniforms.uHasMask.value = false; }
            }
        } else {
            // console.log("TryOnRenderer: No segmentationMasks found in mediaPipeResults.");
            if (segmentationTextureRef.current) { segmentationTextureRef.current.dispose(); segmentationTextureRef.current = null; }
             if (effectPassRef.current) { effectPassRef.current.uniforms.uSegmentationMask.value = null; effectPassRef.current.uniforms.uHasMask.value = false; }
        }
    ...
    // --- Handle Resizing (Including Composer's Render Target) - No change ---
    ...
    // --- Scale Base Plane to Fit Viewport - No change ---
