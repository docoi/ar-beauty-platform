// src/components/TryOnRenderer.jsx - CORRECTED SYNTAX for currentIntensity + RenderTarget Mipmap Fix Attempt
...
const TryOnRenderer = forwardRef(({
    ...
    const currentBrightness = useRef(1.0); // Kept for potential future use
    const currentContrast = useRef(1.0);   // Kept for potential future use
    // *** Ensure currentIntensity ref is defined ***
    const currentIntensity = useRef(0.5);
    // *******************************************
    const renderLoopCounter = useRef(0);
    const lastMaskUpdateTime = useRef(0);

    // Add a log right after definition to be sure
    // console.log("TryOnRenderer: currentIntensity ref created, initial value:", currentIntensity.current);
    ...
    const postFragmentShader = `
        ...
        // Removed unused uniforms (uSegmentationMask, uHasMask) for this specific debug shader
    ...
    useEffect(() => { // Update intensity ref from prop
        // console.log("TryOnRenderer: Updating currentIntensity ref to:", effectIntensity);
    ...
    useEffect(() => { // Mask Texture Creation (Keep attempting to create it)
    ...
    // --- Render Loop --- (Use Force Red Shader)
     const renderLoop = useCallback(() => {
        ...
        try {
            ...
            // Check uniforms needed by current "Force Red" shader
            if (!postUniforms?.uSceneTexture || !postUniforms?.uEffectIntensity) { return; }
            ...
            // 5. Update Post-Processing Uniforms (Only SceneTexture and Intensity for this shader)
             postUniforms.uSceneTexture.value = renderTargetRef.current.texture;
             postUniforms.uEffectIntensity.value = currentIntensity.current; // Use the ref updated by props

             // Keep assigning mask uniforms even though shader isn't using them,
             // avoids errors if material expects them.
    ...
    // --- Initialize Scene --- (RenderTarget Mipmap Fix Attempt)
    const initThreeScene = useCallback(() => {
        if (!canvasRef.current || isInitialized.current) return;
        console.log("DEBUG: initThreeScene START (Force Effect Debug + Mipmap Fix)");
        try {
            ...
            postMaterialRef.current = new THREE.ShaderMaterial({
                ...
                uniforms: {
                    ...
                    uSegmentationMask: { value: null }, // Define but unused by current shader
                    uEffectIntensity: { value: currentIntensity.current }, // Needed by Force Red shader
                    uHasMask: { value: false }, // Define but unused by current shader
                },
