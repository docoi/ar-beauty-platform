// src/components/TryOnRenderer.jsx - Force White Material, Force Update
...
const TryOnRenderer = forwardRef(({ videoWidth, videoHeight, className }, ref) => {
  ...
  const imageTextureRef = useRef(null); // Keep ref for cleanup
  ...
  const initThreeScene = useCallback(() => {
    ...
    try {
      ...
      // *** Use BASIC MeshBasicMaterial - Start with WHITE color ***
  ...
  useImperativeHandle(ref, () => ({
    renderResults: (videoElement, results) => { // For Mirror
        ...
        try {
            ...
            // Assign map and force update
            if (material.map !== videoTextureRef.current) {
                 material.map?.dispose(); // Dispose old map if different
                 material.map = videoTextureRef.current;
            }
             // *** Force material update every frame in mirror mode ***
    ...
    renderStaticImageResults: (imageElement, results) => { // For Selfie (Keep NOOP)
