...
const RealTimeMirror = forwardRef(({
  ...
  useEffect(() => {
    ...
    enableStream();
    return () => { isMounted = false; cancelAnimationFrame(animationFrameRef.current?.rafId); videoStream?.getTracks().forEach(track => track.stop()); if (videoRef.current) { videoRef.current.srcObject = null; videoRef.current.onloadedmetadata = null; videoRef.current.onerror = null; } setVideoStream(null); setIsCameraLoading(true); };
   }, [faceLandmarker]); // Depends on landmarker for readiness check
  ...
  }, [faceLandmarker, imageSegmenter]); // Depends on both models
      ...
      <div className="relative w-full max-w-md mx-auto" style={{ paddingTop: `${videoDimensions.width > 0 ? (videoDimensions.height / videoDimensions.width) * 100 : 75}%` }}>
        ...
        {shouldRenderTryOn ? (
          <TryOnRenderer
            ...
            imageElement={null}
            mediaPipeResults={latestLandmarkResults} // <<< Pass landmarks results
            segmentationResults={latestSegmentationResults} // <<< Pass segmentation results
            isStatic={false}
            // brightness={1.0} contrast={1.0} // Removed as TryOnRenderer doesn't use them now
